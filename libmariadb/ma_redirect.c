    ...
    ulong client_flag)
{
    if (mysql->options.enable_redirect != REDIRECTION_OFF)
    {
        ...
        {
            MYSQL* ret_mysql = mysql_real_connect(mysql,
                info->host,
                info->user,
                passwd,
                db,
...
MYSQL* redirect(MYSQL* mysql, int client_flag)
{
    ...
    if (mysql->options.enable_redirect != REDIRECTION_OFF)
    {
        ...
        if (!(info = parse_redirection_info(mysql)))
        {
            ...
            {
                my_set_error(mysql, ER_BAD_HOST_ERROR, SQLSTATE_UNKNOWN,
        ...
        mysql_init(&redirect_mysql);
        redirect_mysql.options = mysql->options;
        void* old_context = redirect_mysql.options.extension->async_context;
        redirect_mysql.options.extension->async_context = NULL;

        if (!mysql_real_connect(&redirect_mysql, info->host, info->user, mysql->passwd,
            ...
            mysql->client_flag | CLIENT_REMEMBER_OPTIONS))
        {
            ...
            if (mysql->options.enable_redirect == REDIRECTION_ON)
            {
                my_set_error(mysql, redirect_mysql.net.last_errno,
                    redirect_mysql.net.sqlstate,
                    redirect_mysql.net.last_error);
                mysql_close(&redirect_mysql);
                return NULL;
            }
            else
            {
        ...
        mysql->free_me = 0;
        mysql_close(mysql); // mysql_close contains end_server(in mysql_close_slow_part), so no need to end it again
